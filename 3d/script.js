...
scene.background = new THREE.Color(0xFFE58A);
...
const hemilLight = new THREE.HemisphereLight(0xffffff, 0xffffff, 1);
hemilLight.position.set(0, 25, 0);
  ...
  model.traverse((node) => {
    if (node.isMesh) {
      if (node.material) {
        ...
        node.material.envMapIntensivity = 1.5;
      }
...
const scannerPostion = scannerSection.offsetTop;
...
function animate() {
  if (model) {
    ...
    const scrollProgress = Math.min(currentScroll / scannerPostion, 1);
